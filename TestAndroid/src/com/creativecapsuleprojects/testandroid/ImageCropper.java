...
import com.creativecapsuleprojects.testandroid.utility.Common;
...
public class ImageCropper extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		sourceUri = getIntent().getExtras().getParcelable(IMAGE_CROPPER_SOURCE_URI);
		targetUri = getIntent().getExtras().getParcelable(IMAGE_CROPPER_TARGET_URI);;
		Log.d(DEBUG_TAG, "output URI:" + sourceUri.getEncodedPath());
		outputWidth = getIntent().getExtras().getInt(IMAGE_CROPPER_OUTPUT_X);
		outputShape = getIntent().getExtras().getInt(
				IMAGE_CROPPER_OUTPUT_SHAPE, outputShapeTypeSquare);
		if (outputShape == outputShapeTypeRectangle) {
			outputHeight = getIntent().getExtras().getInt(
					IMAGE_CROPPER_OUTPUT_Y);
		} else {
		...
		try {
			...
			sourceBitmap = Common.obtainBitmapFromUri(sourceUri, this);
			if (sourceBitmap.getWidth() > 1024) {
				BitmapFactory.Options options = new BitmapFactory.Options();
				options.inSampleSize = 4;

				int width = 1024;
				int height = (int) ((float) sourceBitmap.getHeight()
						/ sourceBitmap.getWidth() * width);
				Log.d(DEBUG_TAG, "source width:" + sourceBitmap.getWidth()
						+ " height:" + sourceBitmap.getHeight());
				Log.d(DEBUG_TAG, "scaled width:" + width + " height:" + height);

				scaledBitmap = Bitmap.createScaledBitmap(sourceBitmap, width,
						height, false);
		...
		ViewTreeObserver imagePreviewObserver = sourceImagePreview
				.getViewTreeObserver();
		imagePreviewObserver
				.addOnGlobalLayoutListener(new OnGlobalLayoutListener() {
					...
					public void onGlobalLayout() {
						...
						Log.d(DEBUG_TAG, "Image frame x:" + sourceFrameX
								+ " y:" + sourceFrameY + " width:"
								+ sourceFrameWidth + " height:"
								+ sourceFrameHeight);
		...
		saveBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Log.d(DEBUG_TAG,
						"crop mask frame x:" + imageCropMask.getLeft() + " y:"
								+ imageCropMask.getTop() + " width:"
								+ imageCropMask.getWidth() + " height:"
								+ imageCropMask.getHeight());
				float frame[] = sourceImagePreview.getImageFrame();
				boolean successfullyWritten = false;

				int x, y, w, h;
				x = (int) ((imageCropMask.getLeft() - frame[0])
						* (float) sourceBitmap.getWidth() / frame[2]);
				y = (int) ((imageCropMask.getTop() - frame[1])
						* (float) sourceBitmap.getHeight() / frame[3]);
				w = (int) (imageCropMask.getWidth() / frame[2] * (float) sourceBitmap
						.getWidth());
				h = (int) (imageCropMask.getHeight() / frame[3] * (float) sourceBitmap
						.getHeight());

				Log.d(DEBUG_TAG,
						"cropped view frame x:"
								+ (imageCropMask.getLeft() - frame[0]) + " y:"
								+ (imageCropMask.getTop() - frame[1])
								+ " width:" + imageCropMask.getWidth()
								+ " height:" + imageCropMask.getHeight());
				Log.d(DEBUG_TAG, "source Image view frame x:" + frame[0]
						+ " y:" + frame[1] + " width:" + frame[2] + " height:"
						+ frame[3]);
				Log.d(DEBUG_TAG, "source Image crop frame x:" + x + " y:" + y
						+ " width:" + w + " height:" + h);
				...
				if (outputShape == outputShapeTypeCircle) {
					croppedBitmap = cropCircularImage1(croppedBitmap);
				}
				try {
					// write to output URI
					...
					Log.w("TAG",
							"Error saving image file FileNotFoundException: "
									+ e.getMessage());
					e.printStackTrace();
				} catch (IOException e) {
					Log.w("TAG",
							"Error saving image file IOException: "
									+ e.getMessage());
					e.printStackTrace();
				}
	...
	View.OnLayoutChangeListener layoutChangedListener = new OnLayoutChangeListener() {
		...
		public void onLayoutChange(View view, int left, int top, int right,
			...
			if (view == sourceImagePreview) {
				...
				sourceFrameWidth = sourceImagePreview.getWidth();
				sourceFrameHeight = sourceImagePreview.getHeight();
				Log.d(DEBUG_TAG, "Image o=LayoutChange frame x:" + sourceFrameX
						+ " y:" + sourceFrameY + " width:" + sourceFrameWidth
	...
	private Bitmap cropCircularImage1(Bitmap source) {
		Bitmap output = Bitmap.createBitmap(source.getWidth(),
				source.getHeight(), Config.ARGB_8888);
	    Canvas canvas = new Canvas(output);

	    final int color = 0xffCECECE;
	...
	private Bitmap cropCircularImage2(Bitmap source) {
	...
	private Bitmap cropCircularImage3(Bitmap source) {
		Bitmap targetBitmap = Bitmap.createBitmap(source.getWidth(), source.getHeight(),
				Bitmap.Config.ARGB_8888);

		Canvas canvas = new Canvas(targetBitmap);
		BitmapShader shader;
		shader = new BitmapShader(source, BitmapShader.TileMode.CLAMP,
		...
		RectF rect = new RectF(0.0f, 0.0f, source.getWidth(),
				source.getHeight());

		// rect contains the bounds of the shape
		// radius is the radius in pixels of the rounded corners
		// paint contains the shader that will texture the shape
		canvas.drawRoundRect(rect, source.getWidth()/2, source.getWidth()/2, paint);
		return targetBitmap;
	}
